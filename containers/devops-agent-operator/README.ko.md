...
유효한 값: `ContainerWaiting`, `ContainerTerminated`, `PodPhase`, `PodStatus`, `PodCondition` ([감지 레이어 상세](docs/ko/ARCHITECTURE.md#3단계-장애-감지---detectpodfailure))
...
유효한 값 (낮을수록 심각): `CRITICAL`, `HIGH`, `MEDIUM`, `LOW` ([심각도별 장애 유형](docs/ko/ARCHITECTURE.md#6단계-심각도-결정---determineseverity))
